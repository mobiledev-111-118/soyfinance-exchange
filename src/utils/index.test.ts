...
import { TokenAmount, Token, ChainId, Percent, JSBI } from '@soy-libs/sdk'
...
describe('utils', () => {
  describe('#getEtherscanLink', () => {
    it('correct for tx', () => {
      expect(getCallistoScanLink(1, 'abc', 'transaction')).toEqual('https://explorer.callisto.network/tx/abc')
    })
    it('correct for token', () => {
      expect(getCallistoScanLink(1, 'abc', 'token')).toEqual('https://explorer.callisto.network/token/abc')
    })
    it('correct for address', () => {
      expect(getCallistoScanLink(1, 'abc', 'address')).toEqual('https://explorer.callisto.network/address/abc')
    })
    it('unrecognized chain id defaults to mainnet', () => {
      expect(getCallistoScanLink(2, 'abc', 'address')).toEqual('https://explorer.callisto.network/address/abc')
    })
    it('ropsten', () => {
      expect(getCallistoScanLink(3, 'abc', 'address')).toEqual('https://explorer.callisto.network/address/abc')
    })
    it('enum', () => {
      expect(getCallistoScanLink(ChainId.CLOTESTNET, 'abc', 'address')).toEqual('https://explorer.callisto.network/address/abc')
    })
  })
